...
public class MemberGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all members").get("/api/members").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new member")
                        .post("/api/members")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nickName\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created member").get("${new_member_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created member").delete("${new_member_url}").headers(headers_http_authenticated))
                .pause(10)
        );
